...
class Large(Medium):
    ...
    async def embed(self) -> Embed:
        ...
        field_name = t(
            f"embed.field.{'members' if self.server.game_id == 'discord' else 'player_list'}.name",
            self.locale,
        )
    ...
    def add_player_list_fields(
        ...
        if players is None:
            embed.add_field(
                name=field_name, value="The game does not support this feature."
            )
            return embed
        ...
        for i, player in enumerate(filtered_players):
            ...
            remaining_players_message = (
                f"... {player_left} more player{'' if player_left <= 1 else 's'}"
            )
